...
// Copyright (c) 2019 Lily(Katano-Sukune)
...
import (
	...
	rpc "github.com/Katano-Sukune/xpcd/rpcclient"
...
var (
	...
	connectFlag = flagset.String("s", "localhost", "host[:port] of XPChain Core wallet RPC server")
...
// XPChain transactions for these swaps.  A second tool should be used for the
// transaction on the other chain.  Any chain can be used so long as it supports
// OP_SHA256 and OP_CHECKLOCKTIMEVERIFY.
//
// Example scenerios using xpchain as the second chain:
//
// Scenerio 1:
//   cp1 initiates (dcr)
//   cp2 participates with cp1 H(S) (xpc)
//   cp1 redeems xpc revealing S
...
//     - must verify H(S) in contract is hash of known secret
//   cp2 redeems xpc with S

func init() {
	flagset.Usage = func() {
		fmt.Println("Usage: xpcatomicswap [flags] cmd [cmd args]")
		fmt.Println()
...
type initiateCmd struct {
	cp2Addr *xpcutil.AddressPubKeyHash
	amount  xpcutil.Amount
}

type participateCmd struct {
	cp1Addr    *xpcutil.AddressPubKeyHash
	amount     xpcutil.Amount
	secretHash []byte
}
...
func run() (err error, showUsage bool) {
	...
	switch args[0] {
	case "initiate":
		cp2Addr, err := xpcutil.DecodeAddress(args[1], chainParams)
		if err != nil {
		...
		cp2AddrP2PKH, ok := cp2Addr.(*xpcutil.AddressPubKeyHash)
		if !ok {
		...
		amount, err := xpcutil.NewAmount(amountF64)
		if err != nil {
			return err, true
		}
	...
	case "participate":
		cp1Addr, err := xpcutil.DecodeAddress(args[1], chainParams)
		if err != nil {
		...
		cp1AddrP2PKH, ok := cp1Addr.(*xpcutil.AddressPubKeyHash)
		if !ok {
...
// pubkey for a transaction input signature.  Due to limitations of the XPChain
// Core RPC API, this requires dumping a private key and signing in the client,
// rather than letting the wallet sign.
func createSig(tx *wire.MsgTx, idx int, pkScript []byte, addr xpcutil.Address,
...
// xpcd/rpcclient package.
func fundRawTransaction(c *rpc.Client, tx *wire.MsgTx, feePerKb xpcutil.Amount) (fundedTx *wire.MsgTx, fee xpcutil.Amount, err error) {
	...
	feeAmount, err := xpcutil.NewAmount(resp.Fee)
	if err != nil {
		return nil, 0, err
	}
...
func getFeePerKb(c *rpc.Client) (useFee, relayFee xpcutil.Amount, err error) {
	...
	relayFee, err = xpcutil.NewAmount(netInfoResp.RelayFee)
	if err != nil {
		return 0, 0, err
	}
	payTxFee, err := xpcutil.NewAmount(walletInfoResp.PayTxFee)
	if err != nil {
		return 0, 0, err
	}
	...
	if err == nil && estimateResp.FeeRate > 0 {
		useFee, err = xpcutil.NewAmount(estimateResp.FeeRate)
		if relayFee > useFee {
			useFee = relayFee
		}
...
func getRawChangeAddress(c *rpc.Client) (xpcutil.Address, error) {
	...
	addr, err := xpcutil.DecodeAddress(addrStr, chainParams)
	if err != nil {
		return nil, err
	}
	...
	if _, ok := addr.(*xpcutil.AddressPubKeyHash); !ok {
...
type contractArgs struct {
	them       *xpcutil.AddressPubKeyHash
	amount     xpcutil.Amount
	locktime   int64
	secretHash []byte
}
...
type builtContract struct {
	contract       []byte
	contractP2SH   xpcutil.Address
	contractTxHash *chainhash.Hash
	contractTx     *wire.MsgTx
	contractFee    xpcutil.Amount
	refundTx       *wire.MsgTx
	refundFee      xpcutil.Amount
}
...
func buildContract(c *rpc.Client, args *contractArgs) (*builtContract, error) {
	...
	contractP2SH, err := xpcutil.NewAddressScriptHash(contract, chainParams)
	if err != nil {
		return nil, err
	}
...
func buildRefund(c *rpc.Client, contract []byte, contractTx *wire.MsgTx, feePerKb, minFeePerKb xpcutil.Amount) (
	refundTx *wire.MsgTx, refundFee xpcutil.Amount, err error) {
	...
	refundAddr, err := xpcutil.NewAddressPubKeyHash(pushes.RefundHash160[:], chainParams)
	if err != nil {
		return nil, 0, err
	}
	...
	if txrules.IsDustOutput(refundTx.TxOut[0], minFeePerKb) {
		return nil, 0, fmt.Errorf("refund output value of %v is dust", xpcutil.Amount(refundTx.TxOut[0].Value))
	}
...
func calcFeePerKb(absoluteFee xpcutil.Amount, serializeSize int) float64 {
...
func (cmd *initiateCmd) runCommand(c *rpc.Client) error {
	...
	fmt.Printf("Contract fee: %v (%0.4f xpc/kB)\n", b.contractFee, contractFeePerKb)
	fmt.Printf("Refund fee:   %v (%0.4f xpc/kB)\n\n", b.refundFee, refundFeePerKb)
...
func (cmd *redeemCmd) runCommand(c *rpc.Client) error {
	...
	recipientAddr, err := xpcutil.NewAddressPubKeyHash(pushes.RecipientHash160[:],
		chainParams)
	if err != nil {
		return err
	}
	contractHash := xpcutil.Hash160(cmd.contract)
	contractOut := -1
	for i, out := range cmd.contractTx.TxOut {
		...
		if sc == txscript.ScriptHashTy &&
			bytes.Equal(addrs[0].(*xpcutil.AddressScriptHash).Hash160()[:], contractHash) {
			contractOut = i
			break
		}
	}
	...
	if txrules.IsDustOutput(redeemTx.TxOut[0], minFeePerKb) {
		return fmt.Errorf("redeem output value of %v is dust", xpcutil.Amount(redeemTx.TxOut[0].Value))
	}
	...
	fmt.Printf("Redeem fee: %v (%0.4f xpc/kB)\n\n", fee, redeemFeePerKb)
...
func (cmd *auditContractCmd) runOfflineCommand() error {
	contractHash160 := xpcutil.Hash160(cmd.contract)
	contractOut := -1
	for i, out := range cmd.contractTx.TxOut {
		...
		if bytes.Equal(addrs[0].(*xpcutil.AddressScriptHash).Hash160()[:], contractHash160) {
			contractOut = i
			break
		}
	}
	...
	contractAddr, err := xpcutil.NewAddressScriptHash(cmd.contract, chainParams)
	if err != nil {
		return err
	}
	recipientAddr, err := xpcutil.NewAddressPubKeyHash(pushes.RecipientHash160[:],
	...
	refundAddr, err := xpcutil.NewAddressPubKeyHash(pushes.RefundHash160[:],
		chainParams)
	if err != nil {
		return err
	}
	...
	fmt.Printf("Contract value:          %v\n", xpcutil.Amount(cmd.contractTx.TxOut[contractOut].Value))
